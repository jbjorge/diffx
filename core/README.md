...
#### Async `setState()`
When working with [wrapped setState()](#wrapping-setstate-in-setstate), it is likely that the
situation occurs where a function wants to do some asynchronous work before setting the state.

To enhance tracking of async state in Diffx devtools, it is recommended to wrap asynchronous work
inside a 
